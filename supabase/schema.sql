...
create table user_profiles (
  ...
  security_answer_hash   text,           -- pgcrypto bcrypt hash, never the plaintext answer
...
-- for the paste-able version of this block; kept here too so a fresh apply
-- of this file from scratch is complete on its own).
-- ============================================================================
...
-- ============================================================================
-- Self-service password recovery via a security question — no outbound email
-- is configured for this project. Uses pgcrypto (already enabled) to
-- hash/verify answers and to rewrite auth.users.encrypted_password directly,
-- the same way Supabase's own Auth service stores passwords.
--
-- reset_password_with_answer() is intentionally callable anonymously (the
-- whole point is the outlet isn't logged in) — its only protection is the
-- answer check plus a 5-attempt/15-minute lockout. Error messages are
-- deliberately generic ("Incorrect answer") regardless of whether the email
-- exists, has no question set up, or the answer is wrong, so this can't be
-- used to enumerate valid outlet accounts.
-- ============================================================================
...
-- Operator-only: add or edit a product. p_id null -> create (p_code must be
-- unique, operator-provided); p_id set -> edit — p_code is accepted but
